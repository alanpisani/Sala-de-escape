...
$(document).ready(function () {
  ...
  $("button").click(function () {
    ...
    if (c > 0) {
      $("#alert").html("Coloca todas las piezas!");
    } else {
      var w = 0;
      $.each(ansID, function (obj, value) {
        ...
        if (w == 6) {
          ...
          $("#alert").html("Uhuu proba de vuelta!");
          puzzlerSolved = false;
        }
...
function invalidMessage() {
  document.getElementById("math-puzzle-message").innerHTML =
    "Perdón, su elección es inválida";
}

function fullMessage() {
  document.getElementById("math-puzzle-message").innerHTML =
    "Perdón, ya no hay más posiciones para ocupar!";
}

function solvedMessage() {
  document.getElementById("math-puzzle-message").innerHTML =
    "Genial, lo ha resuelto!";
}

function wrongMessage() {
  document.getElementById("math-puzzle-message").innerHTML =
    "Oh no, lo hizo mal. Vuélvalo a intentar!";
}
...
function resetMathPuzzle() {
  ...
  positionE = void 0;
  positionG = void 0;
  document.getElementById("math-puzzle-a").innerHTML = "Número";
  document.getElementById("math-puzzle-b").innerHTML = "Signo";
  document.getElementById("math-puzzle-c").innerHTML = "Número";
  document.getElementById("math-puzzle-d").innerHTML = "Signo";
  document.getElementById("math-puzzle-e").innerHTML = "Número";
  document.getElementById("math-puzzle-g").innerHTML = "Número";
}
...
// When the user clicks the button, open the modal
//flexboxPuzzleBtn.onclick = function() {            /*Hacen clickeables los minijuegos*/
...
$(function () {
  ...
  $("#mmirror").droppable({
    drop: function (event, ui) {
      ...
      $("#mirror-clue").replaceWith("<p>Ah, genial. Encontraste como leer la nota!</p>");
...
const config = {
  [TYPE.ROOM]: {
    seeds: () => {
      const adjectives = ["chiquita", "grande", "sucia"];
      const colors = ["y fluorescente", "y oscura", "y luminosa", "y resplandeciente"];
      return `Habitación ${_.sample(adjectives)} ${_.sample(colors)}`;
    },
    relations: [
      { type: TYPE.DOOR_OUT, min: 1, max: 2 },
      { type: TYPE.CONTAINER, min: 1, max: 5 },
      { type: TYPE.CAT, min: 1, max: 1, p: 0.2 }
    ]
  },
  [TYPE.CONTAINER]: {
    seeds: ["Congelador", "Heladera", "Nevera"],
    relations: [
  ...
  [TYPE.CORRIDOR]: {
    seeds: ['Corredor', 'Pasillo', 'Pasaje', 'Canaleta', 'Sala'],
  ...
  [TYPE.CAT]: {
    seeds: () => {
      const adjectives = ["majestuoso", "gordo", "vago"];
      const coats = ["", "bi-color", "atigrado", "pardo", "tri-color"]
      const colors = ["oscuro", "blanco", "marron", "pelirrojo", "gris", "canela", ""];
      return `Gato ${_.sample(adjectives)} ${_.sample(coats)} ${_.sample(
        colors
      )}`;
    },
  ...
  [TYPE.THOUGHTS]: {
    seeds: ["Mirada cortante", "Garras", "Cola", "Mirada rara", "Pensamientos destructivos", "Hambriento"],
    relations: []
  }
};
...
const config = {
  [TYPE.ROOM]: {
    ...
    relations: [{type: TYPE.DOOR_OUT, min:1, max: 2}, {type: TYPE.CONTAINER, min:1, max:5}, {type: TYPE.CAT, min:1, max:1, p:0.2}]
  },
  [TYPE.CONTAINER]: {
    seeds: ["Freezer", "Heladera", 'Congelador'],
    relations: [{type: TYPE.OBJECT, min:1, max: 2}, {type: TYPE.EXIT, min:1, max:1, p:(data) => {
      const nb = countParents(data);
      if(nb > 2) {
        return 0.05 * (nb/2);
      } else {
  ...
  [TYPE.OBJECT]: {
    seeds: ["Frijoles", "Zanahorias", "Papas fritas", "Pepinillo", "Arveja", "Frutillas", "Blueberries", "Camaron", "Pizza", "Broccoli", "Espinaca", "Sopa"],
    relations: []
  },
  [TYPE.CORRIDOR]: {
    seeds:  ['Corredor', 'Pasillo', 'Pasaje', 'Canaleta', 'Sala'],
    relations: [{type: TYPE.DOOR_IN, min:1, max: 2}]
  },
  [TYPE.DOOR_IN]: {
    seeds:  ['Puerta'],
    relations: [{type: TYPE.ROOM, min:1, max: 1}]
  ...
  [TYPE.CAT]: {
    seeds: () => {
      ...
      const coats = ["", "bi-color", "atigrado", "pardo", "tri-color"]
      const colors = ["oscuro", "blanco", "marron", "pelirrojo", "gris", "canela", ""];
      return `Gato ${_.sample(adjectives)} ${_.sample(coats)} ${_.sample(colors)}`
    },
    relations: [{type: TYPE.THOUGHTS, min:1, max: 2}]
  },
  [TYPE.THOUGHTS]: {
...
let tid = 0;
const generateThing = (type, parent) => {
  let name = Array.isArray(config[type].seeds) ? _.sample(config[type].seeds) : config[type].seeds();
  return {
    type: type,
    name,
    id: tid++,
...
const generateFromParent = parent => {
  ...
  config[parent.type].relations.forEach(r => {
    let p = r.p ? ((r.p instanceof Function) ? r.p(parent) : r.p) : 1;
    if(r.type == TYPE.EXIT) {console.log(p)}
    if( p === 1 || Math.random() < p ){
      links = links.concat(Array.from({length:_.random(r.min, r.max)}, () => generateThing(r.type, parent)))
    }
  });
  return links;
}
  ...
  template: `
    <div class="thing">
      <div class="flex">
        ...
        <div @click="handle" v-if="[TYPE.OBJECT, TYPE.EXIT, TYPE.THOUGHTS].indexOf(data.type) === -1">
          <i v-if="show" class="fa fa-times"></i>
...
function entrarRanking(){
  document.getElementById('entrar-ranking').disabled = false;                   //funcion que solo activa el boton desactivado "guardar"
}
...
function escapeNow() {
  haveEscaped = setTimeout(function() {
    ...
    document.getElementById("open-door").style.opacity = "1";            //Sin esta funcion, no se puede terminar el juego
    // window.alert("You may now escape!");
    //document.getElementById("escaped").style.visibility = "visible";
    document.getElementById("overlay").style.visibility = "visible";
    document.getElementById("titulo-formulario").innerHTML = "GANASTE!!!";
    document.getElementById("titulo-formulario").style.color = "lightgreen";
    entrarRanking();
...
/* Función para limitar el tiempo de juego */




function lostGame(){
  havenotEscaped = setTimeout(function(){
...
//para activarlo, quitar las barras de arriba

/* Función para ir al Ranking tras completar el juego */

function gotoRanking(){
  salida = setTimeout(function(){
    window.alert('Ver ranking');
    window.location='ranking.php';
